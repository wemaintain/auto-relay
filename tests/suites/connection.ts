...
export function ConnectionTests(suiteName: string): void {
  ...
  describe('Connection', () => {
    ...
    describe('Ordering', () => {
      ...
      it('Should return everything in the collection sorted by DESC value', async () => {
        ...
        for (const recipe of test.data!.recipes) {
          let lastValue = 11 // 11 > 10 which is our max rating
          recipe.ratingsByValue.edges.forEach(
            ({ node }: { node: { id: string; value: string } }) => {
